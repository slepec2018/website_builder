...
export class ImgElement extends AbstractElementComponent {
  _getTemplate() {
    ...
    return `<div class="element image ${content ? `element--uploaded` : `element--image`}" tabindex="0">
              ...
              <div class="img-upload">
                ...
                <input placeholder="Insert image link" type="url" class="content-element">
                <label class="img-upload__label">Download</label>
              </div>
              ${content ? `<img alt src="${content}" class="change-img-btn">` : ``}
            </div>`;
  }
  ...
  _afterCreateElement() {
    ...
    this.getElement().querySelector(`.add-img-btn`).addEventListener(`click`, this._addImgBtnClickHandler.bind(this));
    if (this._data.content) {
      this.getElement().querySelector(`.change-img-btn`).addEventListener(`click`, this._addImgBtnClickHandler.bind(this));
    }
  }
